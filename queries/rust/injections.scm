...
;
; Both inner (//!) and outer (///) doc comments use `injection.combined` so
; multi-line markdown structures (fenced code blocks, lists) parse correctly.
;
; Neovim 0.12 includes `clip_regions` (PR #32549) which clips child parser
; ranges to the actual injection content. This prevents the highlight bleeding
; that previously occurred when /// comments on separate items were combined
; into one markdown tree with gaps spanning Rust code between them.
